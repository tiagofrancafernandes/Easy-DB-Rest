...
export function useDatabase(connectionId: string) {
    ...
    async function fetchTableDetails(tableName: string) {
        ...
        try {
            ...
            return await apiClient<{ data: { columns: Column[] } }>(`/connections/${connectionId}/tables/${tableName}`, { params })
        } finally {
            isLoading.value = false
        }
    }
    ...
    async function fetchSchemas() {
        const params: Record<string, string> = {}
        if (currentDatabase.value) params.database = currentDatabase.value
        const response = await apiClient<{ data: string[] | any[] }>(`/connections/${connectionId}/schemas`, { params })
    ...
    async function fetchTableData(tableName: string, options: { limit?: number, offset?: number } = {}) {
        ...
        try {
            ...
            return await apiClient<{ data: any[], meta: { total: number } }>(`/connections/${connectionId}/tables/${tableName}/data`, { params })
        } finally {
            isLoading.value = false
        }
    }
